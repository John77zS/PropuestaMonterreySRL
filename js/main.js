...
const botKnowledge = [
  ...
  },
  {
    ...
    response:
      "Claro, para ayudarte mejor: ¿de qué parte de Bolivia nos escribes?",
    intent: "ubicacion",
  },
  {
    keywords: [
...
/* =========================
   IMAGEN DINÁMICA DROPDOWN PRODUCTOS
========================= */

const productDropdownLinks = document.querySelectorAll(".products-menu-links a");
const productsMenuImage = document.getElementById("productsMenuImage");

if (productDropdownLinks.length > 0 && productsMenuImage) {
  productDropdownLinks.forEach((link) => {
    link.addEventListener("mouseenter", () => {
      ...
      productsMenuImage.style.opacity = "0";
      productsMenuImage.style.transform = "scale(1.06)";

      setTimeout(() => {
        productsMenuImage.src = newImage;
        productsMenuImage.style.opacity = "1";
        productsMenuImage.style.transform = "scale(1)";
      }, 180);
    });
  });
}
...
/* =========================
   ORGANIGRAMA MOBILE VER MÁS
========================= */

const orgTree = document.querySelector(".org-tree");
const orgToggleBtn = document.getElementById("orgToggleBtn");

if (orgTree && orgToggleBtn) {
  orgToggleBtn.addEventListener("click", () => {
    orgTree.classList.toggle("org-expanded");

    const isExpanded = orgTree.classList.contains("org-expanded");

    orgToggleBtn.innerHTML = isExpanded
      ? 'Ver menos cargos <span>↑</span>'
      : 'Ver más cargos <span>↓</span>';
  });
}